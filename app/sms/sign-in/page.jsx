...
import { signIn, verifyOTP, isPhoneExisted, isExistedInWaitlist } from "./actions";
    ...
    <div className="flex-1 flex flex-col w-full px-8 sm:max-w-xl justify-center gap-2">
      ...
      <form
        action={async (formData) => {
          ...
          if (await isExistedInWaitlist(formData.get('phone').replace(/\D/g, ''))) {
            toast.error('Sorry, you are still on the waitlist. We will let you know when we are ready for you');
            return;
          }

          if ( await isPhoneExisted(formData.get('phone').replace(/\D/g, ''))) {
            toast.error('Sorry, you do not have an account. We have added you to the waitlist.');
            // router.push("/sms/sign-up");
            return;
          }
